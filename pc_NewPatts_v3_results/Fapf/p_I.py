...
def compute_pc(apf_values, p_values, num_datasets):
  
  print(len(apf_values))
  print(len(p_values))
  p_cc = numpy.zeros((num_datasets,len(apf_values),len(p_values)))
  p_c = numpy.zeros((num_datasets,len(apf_values)))
  i=0
  j=0
  data2 = numpy.zeros((num_datasets,len(apf_values),len(p_values)))
  for k in range(0,num_datasets):
    data = (loadtxt('storage_capacity_dataset_%s'%(k)))
    for i in range(0,len(apf_values)):
...
def compute_info(apf_values, p_values, num_datasets, i):
	for k in range(0,num_datasets):
		data = (loadtxt('storage_capacity_dataset_%s'%(k)))
		i_c = []
		pi_c = []
...
color = 'green'
p_values = numpy.arange(10,2000,10)
apf_values = [0.05]
apf_values = numpy.append(apf_values, numpy.linspace(0.1,1.0,10))
print(apf_values)
num_datasets = 1
...
p_c = compute_pc(apf_values, p_values, num_datasets)
plt.errorbar(apf_values, numpy.mean(p_c/Cm, axis = 0), yerr=numpy.std(p_c/Cm, axis= 0), marker= 'o', color=color)

ax.set_xlabel('$a_p$') 
ax.set_ylabel(r'$\alpha_c$')
#ax.set_xticks(apf_values)
#legend = ax.legend(loc='lower center')
plt.xlim(0, 1)
plt.ylim(0, 8)
#plt.grid(True)
plt.savefig('p_c_fapf N=%s Cm=%s S=%s U=%s beta=%s.png'%(N, Cm, S, U, beta ), bbox_inches='tight')
plt.savefig('p_c_fapf N=%s Cm=%s S=%s U=%s beta=%s.pdf'%(N, Cm, S, U, beta ), bbox_inches='tight')


fig = plt.figure(figsize=(5.5, 5))
ax = fig.add_subplot(1,1,1)
#information
for j in range(0,len(apf_values)):
	pvals, info, frac_retr, frac_retr_2ndpatt = compute_info(apf_values, p_values, num_datasets, j)
...
ax2.set_xlabel('$p$') 
ax2.set_ylabel(r'\bf{fraction retrieved}')
plt.xlim(0, 8)
