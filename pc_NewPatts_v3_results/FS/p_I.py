...
def compute_info(S_values, p_values, num_datasets, i):
	for k in range(0,num_datasets):
		data = (loadtxt('apf%.1f_f%.2f/storage_capacity_dataset_%s'%(apf, f, k)))
		i_c = []
		pi_c = []
...
#plt.grid(True)
plt.tight_layout()
plt.savefig('p_c_storage N=%s Cm=%s a=%s U=%s beta=%s.png'%(N, Cm, a, U, beta ))
plt.savefig('p_c_storage N=%s Cm=%s a=%s U=%s beta=%s.pdf'%(N, Cm, a, U, beta ))
...
for j in range(0,len(S_values)):
	pvals, info, frac_retr = compute_info(S_values, p_values, num_datasets, j)
...
plt.tight_layout()
plt.savefig('info N=%s Cm=%s a=%s U=%s beta=%s.png'%(N, Cm, a, U, beta ))
plt.savefig('info N=%s Cm=%s a=%s U=%s beta=%s.pdf'%(N, Cm, a, U, beta ))
...
plt.tight_layout()
plt.savefig('frac retr N=%s Cm=%s a=%s U=%s beta=%s.png'%(N, Cm, a, U, beta ))
plt.savefig('frac retr N=%s Cm=%s a=%s U=%s beta=%s.pdf'%(N, Cm, a, U, beta ))
