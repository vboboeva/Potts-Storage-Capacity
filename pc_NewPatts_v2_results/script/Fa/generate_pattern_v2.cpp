/*----------------------------------------------------------------------*
*		New pattern generator written by Vizhe (v2),							*
* 		that defines factors as random patterns acting on random		*
* 		subsets of patterns	and not units, as does Ale. 				*
...
void GetMemory(double a, int p, int p_fact)
{
int i, mu;
...
void SetFactors(double a, int p, int p_fact)
{
  //char *buffer; 
...
void AssignChildren(double a, int p, int p_fact)
{
...
void SetPatterns(double a, int p, int p_fact)
{
  int i, mu, count;
  for(mu=0;mu<p;mu++)
  {
...
int SavePatterns(double a, int p, int p_fact)
{
int mu, i;
