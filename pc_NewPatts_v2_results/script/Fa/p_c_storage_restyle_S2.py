...
apf = [0.4, 1.0]
colors = ['green', 'blue']
p_values = [numpy.arange(20,340,2),numpy.arange(20,340,2)]
num_datasets = [1,1]
a_values_ticks = numpy.linspace(0.0,1.00,11)
p_values_ticks = numpy.arange(-20,520,20)
...
for i in range(numpy.size(apf)):
  ...
  if apf[i] == 0:
    plt.errorbar(a_values, numpy.mean(p_c/Cm, axis = 0), yerr=numpy.std(p_c/Cm, axis= 0), marker= 'o', label=r"$\displaystyle\ a_{p}=%.1f, f=%.2f$"%(apf[i], 0.0), color=colors[i])
  else:
    plt.errorbar(a_values, numpy.mean(p_c/Cm, axis = 0), yerr=numpy.std(p_c/Cm, axis= 0), marker= 'o', label=r"$\displaystyle\ %.1f, %.2f$"%(apf[i], 0.05), color=colors[i])
	  
data_ana =  (loadtxt("S2_ap04_correlated.txt"))
plt.plot(data_ana[:,0], data_ana[:,1]/Cm, linewidth = 3, linestyle = '--', color='black', label = r"analytic")

data_ana =  (loadtxt("S2_ap10_correlated.txt"))
plt.plot(data_ana[:,0], data_ana[:,1]/Cm, linewidth = 3, linestyle = '--', color='black')
