...
/*pattern parameters*/
extern int retr;
extern int mu_f; // remember to change this
...
void compute_sparsity_by_state(int p)
{
int mu, k, i;	
for(mu=0;mu<p;mu++)
{
...
den=0.;
  for(i=0;i<N;i++)
  {
    ...
    {
	  //printf("%.2f\n",s[i][k]);
      num+=((double)(xi[retr][i]==k)-sparsity_by_state[retr][k])*s[i][k]; //to calculate m
      den+=((double)(xi[retr][i]==k)-sparsity_by_state[retr][k])*(double)(xi[retr][i]==k); //to calculate m  
    }
  }
m_retr=num/den;
//printf("num=%.2f\n", num);
...
//{
  ...
  //for(k=0;k<S;k++)
  //{
    //ma+=((double)(xi[retr][i]==k)-a/(float)S)*s[i][k]; //to calculate m  
  //}
  //maa+=ma;
//}
//m_retr=maa*invdenN; //value of m[mu] for each mu
...
/*-----------------------------------------------------------*/
/*constructs C and J*/
void construct_CJ(double a, int p)
{

int i, j, l, k, mu;
int i_c, x, new_one;
double denCm;
denCm=a*(1.-a/S)*(double)Cm;
//for(i=0; i<N; i++)
//{
  //for(j=0; j<N; j++)
  //{
...
/*-----------------------------------------------------------*/
/*initializes network*/
void initializing(double a, int p)
{
int i, j, k;
...
double Z, invZ, hmax, rmax, smax, rho, cue_field;	
  ...
  if (update_h == 1)
  {
    ...
    //   if(t >= 9.0){fprintf(field_file, "%f\n", h[i][k]);}
      //printf("%d\t", retr);
      h[i][k] = h[i][k] + (exp_cue==1)*cue_field*units_receive_cue[i]*(xi[retr][i]==k);
  ...
  {  
    ...
    //   if(t >= 9.0){fprintf(field_file, "%f\n", h[i][k]);}
      /*adds cue_field to h only if retr_i is in state k and global noise*/
      rho = 2*drand48()-1;
      h[i][k] += cue_field*units_receive_cue[i]*(xi[retr][i]==k) + nu*rho;
      /*update r and theta*/
