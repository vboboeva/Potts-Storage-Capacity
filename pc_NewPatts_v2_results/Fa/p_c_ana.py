...
prob = [0.00, 0.05]
colors = ['black', 'green']
p_values = [numpy.arange(10,2000,10), numpy.arange(20,1020,10)]
print(p_values)
num_datasets = [4, 1]
...
for i in range(numpy.size(apf)):
  ...
  print("p_c=", p_c/Cm) 
  plt.errorbar(a_values, numpy.mean(p_c/Cm, axis = 0), yerr=numpy.std(p_c/Cm, axis= 0), marker= 'o', label=r"$\displaystyle\ a_{p}=%.1f, f=%.2f $"%(apf[i], prob[i]), color=colors[i])

fvals = (loadtxt('corrs_fa.txt'))[:,0]
pcvals = (loadtxt('corrs_fa.txt'))[:,4]
plt.plot(fvals, pcvals, ls='-.', color='green')

fvals = (loadtxt('Inter_0.1_fixedpt.txt'))[:,0]
pcvals = (loadtxt('Inter_0.1_fixedpt.txt'))[:,1]
plt.plot(fvals, pcvals, ls='-.', color='black')
...
#plt.grid(True)
plt.savefig('p_c_corr_fa_ana N=%s Cm=%s S=%s U=%s beta=%s.png'%(N, Cm, S, U, beta ), bbox_inches='tight')
plt.savefig('p_c_corr_fa_ana N=%s Cm=%s S=%s U=%s beta=%s.pdf'%(N, Cm, S, U, beta ), bbox_inches='tight')
plt.close()
