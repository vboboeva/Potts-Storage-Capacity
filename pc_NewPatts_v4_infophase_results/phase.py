...
z_size = numpy.size(zeta_values)
f_size = numpy.size(f_values)
#
xsize=4*1.62
ysize=4
...
# points = numpy.zeros((f_size*z_size,2))
# values = numpy.zeros(f_size*z_size)
...
# for i in range(0, len(zeta_values)):
	# for j in range(0, len(f_values)):
		...
		# if (zeta_values[i] == 0.0):
			# test = numpy.append(test,numpy.mean(info[x]))		
		# points[k,0] = zeta_values[i]
		# points[k,1] = f_values[j]
		# #print(info[x])
		# if (info[x] != []):
			# indices = numpy.where(info[x] == float('inf'))
			# noninf = numpy.delete(info[x], indices)
			# values[k] = numpy.mean(noninf)
		# else:
			# values[k]
		# print(zeta_values[i], f_values[j], values[k])		
		# k=k+1

# print(len(zeta_values),len(f_values))
# smth = numpy.reshape(values, (len(zeta_values),len(f_values)))
# numpy.savetxt("points.dat", points, fmt='%.4f')
# numpy.savetxt("values.dat", values, fmt='%.4f')

points = loadtxt('points.dat')
values = loadtxt('values.dat')

fig = plt.figure(figsize=(xsize, ysize))
grid_svalues, grid_pvalues = numpy.meshgrid(numpy.linspace(min(zeta_values), max(zeta_values), 10000), numpy.linspace(min(f_values), max(f_values), 1000))
grid = griddata(points, values, (grid_svalues, grid_pvalues), method='linear')
plt.imshow(grid/Cm, origin="lower", cmap=cm.jet, extent=[min(zeta_values),max(zeta_values),min(f_values),max(f_values)], interpolation="bilinear", aspect="auto") #
#plt.scatter(numpy.repeat(zeta_values, len(f_values)), numpy.tile(f_values, len(zeta_values)), 50, values, cmap=cm.jet) #, edgecolor='black'
#plt.grid(True)
...
plt.ylabel(r'$f$')
plt.show()
#fig.savefig("phase_resI_zeta_vs_f.png", bbox_inches='tight')
#fig.savefig("phase_resI_zeta_vs_f.pdf", bbox_inches='tight')
