...
#define Runs 1
#define N 2000
#define corr 0.14792818151484824	
#define myconst 1.
double alpha;
...
void find_fixed_pt(){
	...
	{    
		...
		psi = o/(1.-o);
		rho = sqrt(alpha*q*a*(1. + myconst*N*alpha*cdivN*corr*(corr-a/(double)S)/(a*(1-a/(double)S)))*(1. + 2.*cdivN*psi + cdivN*psi*psi)/((double)S*S*(1.-a/(double)S)));
		...
		while(ts<T && check==0)
		{
			...
			o = (1.-eta)*o_old + eta*Fo/(sqrt(a*(1-a/(double)S)*alpha*q_old*(1. + myconst*N*alpha*cdivN*corr*(corr-a/(double)S)/(a*(1.-a/(double)S)))*(1. + 2.*cdivN*psi + cdivN*psi*psi)));
