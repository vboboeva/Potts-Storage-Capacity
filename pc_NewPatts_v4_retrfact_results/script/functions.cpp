...
extern double	*mf; 
extern double	*mf_sorted; 
extern int		*indices_f;
...
/*compare function for sorting algorithm*/
int compare_mf(const void *x, const void *y)
{
  int ix = *(int *)x;
  int iy = *(int *)y;
  return mf_sorted[ix] > mf_sorted[iy] ? -1 : mf_sorted[ix] < mf_sorted[iy]; // decreasing order
}
...
qsort(indices, p, sizeof (*indices), compare); // sorts indices, keeping track of indices

for(mu=0;mu<p;mu++)
{
  m_sorted[mu] = m_sorted[indices[mu]];
  //printf("%f\t", m_sorted[mu]);
}
}
...
/*-----------------------------------------------------------*/
/*computes overlap of network state with each PARENT*/

void compute_m_parents()
{
int  k, i, mu;
double ma, maa, invdenN;

invdenN=1./(1*(1.-a/S)*(double)N); // normalizing factor of overlap

for(mu=0;mu<Num_fact;mu++)
{
  mf[mu]=0;	
  maa=0.;
  for(i=0;i<N;i++)
  {
    ...
    for(k=0;k<S;k++)
    {
      ma+=((double)(Factors[mu][i]==k)-a/(float)S)*s[i][k]; //to calculate m  
    }
    maa+=ma;
  }
  mf[mu]=maa*invdenN; //value of m[mu] for each mu
  //printf("%f\t",mf[mu]);
}
}

/*-----------------------------------------------------------*/
/*sorts overlap with PARENTS*/

void compute_m_parents_sorted()
{
int mu;
for(mu=0;mu<Num_fact;mu++)
{
...
memcpy(mf_sorted, mf, sizeof(double)*Num_fact); // copies overlaps inside global array m_sorted
qsort(indices_f, Num_fact, sizeof (*indices_f), compare_mf); // sorts m_sorted, keeping track of indices

for(mu=0;mu<Num_fact;mu++)
{
  mf_sorted[mu] = mf_sorted[indices_f[mu]];
  //printf("%f\t", m_sorted[mu]);
}
}

/*-----------------------------------------------------------*/
/*compare overlap child with parent*/

void test_m_mf()
{

printf("%.2f\t%.2f\n", m_sorted[0], mf_sorted[0]);

if(m_sorted[0] > mf_sorted[0] and indices[0] == cue)
{
	printf("1\n");
	retr = cue;
	m_retr = m_sorted[0];
...
else if(m_sorted[0] > mf_sorted[0] and indices[0] != cue)
{
	printf("2\n");
	retr = indices[0];
	m_retr = m_sorted[0];
...
else //if(m_sorted[0] < mf_sorted[0])
{
	printf("3\n");
    retr_f = indices_f[0];
    fraction_fact_each[retr_f] = fraction_fact_each[retr_f]+1;
	m_retr = mf_sorted[0];
	m_cue = m[cue];
}
}
...
/*--------------------------------------------------------*/
/* prints final configuration of network  */
...
CP = new int*[p];
for(i=0; i<p; i++)
  CP[i] = new int[Num_fact];
  
mf = new double[Num_fact];
mf_sorted = new double[Num_fact];  
  
indices = new int[p];

indices_f = new int[Num_fact];

fraction_fact_each = new int[Num_fact];
