...
# clusters with a single element
i_glass = [11]
i_watch = [10]
i_key = [9]
i_apartment = [8]
...
all_clusters = [i_truck_arm, i_tomato_eye, i_lettuce_cow, i_refr_screw, i_car_beetle, i_pants_shirt, i_bike_train, i_glass, i_watch, i_key, i_apartment, i_ant, i_chimney, i_hammer, i_carrot]
...
for i in range(15):
	for j in range(15):
		if (i!=j):
			couples = list(product(all_clusters[i][:], all_clusters[j][:]))
			print(couples)
			mylist = []
			for couple in couples:
				#print(couple)
				mylist = numpy.append(mylist, C[couple[0],couple[1]])
			#print(mylist)
			meanval = numpy.mean(mylist)
			for couple in couples:
				C[couple[0],couple[1]] = meanval
numpy.savetxt('C_feats_shuffled_within_clusters_intercluster_mean.dat', C, fmt='%.4f')
...
fig.savefig('C_clustered_shuffled_within_%s_intercluster_mean.pdf'%N_shuffle, bbox_inches='tight')
fig.savefig('C_clustered_shuffled_within_%s_intercluster_mean.png'%N_shuffle, bbox_inches='tight')
