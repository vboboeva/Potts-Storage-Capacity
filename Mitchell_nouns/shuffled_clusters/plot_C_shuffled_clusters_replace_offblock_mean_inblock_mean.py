...
mean_truck_arm, mean_tomato_eye, mean_lettuce_cow, mean_refr_screw, mean_car_beetle, mean_bike_train, mean_else, mean_pants_skirt, mean_veggies = [],[],[],[],[],[],[],[],[]
...
for i in range(N_words):
	for j in range(N_words):
		if (i in i_truck_arm and j in i_truck_arm):
			...
			jvals = numpy.append(jvals,j) 	
			if (i!=j):
				mean_else = numpy.append(mean_else, C[i,j])
			...
			jvals = numpy.append(jvals,j) 			
			if (i!=j):
				mean_pants_skirt = numpy.append(mean_pants_skirt, C[i,j])
			...
			jvals = numpy.append(jvals,j) 
			if (i!=j):
				mean_veggies = numpy.append(mean_veggies, C[i,j])
...
for i in range(N_words):
	for j in range(N_words):
		...
		elif (i  in i_else and j  in i_else and i!=j): 
			C[i,j] = numpy.mean(mean_else)
				
		elif (i in i_pants_skirt and j in i_pants_skirt and i!=j): 
			C[i,j] = numpy.mean(mean_pants_skirt)
			
		elif (i in i_veggies and j in i_veggies and i!=j):
			C[i,j] = numpy.mean(mean_veggies)		
